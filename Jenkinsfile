pipeline {
    ...
    stages {
        ...
        stage('Validate student.json Data & Form Rules') {
            steps {
                echo 'Starting automated test suite directly from Jenkinsfile...'
                
                // Embedded Node.js script to test student.json against form rules
                node {
                    sh '''
                    node -e "
                    const fs = require('fs');
                    ...
                    testCases.forEach((data, index) => {
                        ...
                        // Validation logic matching your HTML constraints
                        ...
                        console.log(\`Input Data -> User: '\${data.username}', Email: '\${data.email}', Phone: '\${data.phone}', Branch: '\${data.branch}'\`);
                    ...
                    if (failed > 0) {
                        process.exit(1); // Fail the Jenkins build
                    }
                    "
                    '''
                }
